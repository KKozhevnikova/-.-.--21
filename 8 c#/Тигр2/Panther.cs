...
namespace Тигр2
{
    public class Panther : Predator, IComparable<Panther>
    {
        protected int startX;
        protected int startY;
        public int size;
        public bool mustache;
        public int CompareTo(Panther other)
        {
            if(this.size!=other.size)
            {
                return this.size.CompareTo(other.size);
            }
            if (this.mustache != other.mustache)
            {
                return this.mustache.CompareTo(other.mustache);
            }
            return 0;
        }
        ...
        public Panther(string info)
        {
            string[] strs = info.Split(';');
            if (strs.Length == 4)
            {
                claws = Convert.ToBoolean(strs[0]);
                size = Convert.ToInt32(strs[1]);
                sex = strs[2];
        ...
        protected virtual void tig(Graphics g)
        {
            ...
            Pen minipen = new Pen(colorBody, 2);
